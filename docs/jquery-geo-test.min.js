...
(function(c){var C=Number.POSITIVE_INFINITY,G=Number.NEGATIVE_INFINITY;c.geo={_allCoordinates:function(e){e=this._flatten(e);for(var g=0,j=[];g<e.length;g++){var h=e[g].coordinates,n=c.isArray(h[0]),t=n&&c.isArray(h[0][0]),l;if(!(t&&c.isArray(h[0][0][0]))){if(!t){n||(h=[h]);h=[h]}h=[h]}for(n=0;n<h.length;n++)for(t=0;t<h[n].length;t++)for(l=0;l<h[n][t].length;l++)j.push(h[n][t][l])}return j},_center:function(e){return[(e[0]+e[2])/2,(e[1]+e[3])/2]},_expandBy:function(e,g,j){e=this._center(e);return[e[0]-
g,e[1]-j,e[0]+g,e[1]+j]},_height:function(e){return e[3]-e[1]},_reaspect:function(e,g){var j=this._width(e),h=this._height(e),n=this._center(e);if(j==0||h==0||g<=0)return e;if(j/h>g){j=j/2;h=j/g}else{h=h/2;j=h*g}return[n[0]-j,n[1]-h,n[0]+j,n[1]+h]},_scaleBy:function(e,g){return this._expandBy(e,this._width(e)*g/2,this._height(e)*g/2)},_width:function(e){return e[2]-e[0]},_bbox:function(e){var g=c(e).data("bbox");if(!g)if(e.bbox)c(e).data("bbox",g=e.bbox);else{g=[C,C,G,G];for(var j=this._allCoordinates(e),
h=0;h<j.length;h++){g[0]=Math.min(j[h][0],g[0]);g[1]=Math.min(j[h][1],g[1]);g[2]=Math.max(j[h][0],g[2]);g[3]=Math.max(j[h][1],g[3])}c(e).data("bbox",g)}return g},_contains:function(e,g){if(e.type!="Polygon")return false;switch(g.type){case "Point":return this._containsPolygonPoint(e.coordinates,g.coordinates);case "LineString":return this._containsPolygonLineString(e.coordinates,g.coordinates);case "Polygon":return this._containsPolygonLineString(e.coordinates,g.coordinates[0])}},_containsPolygonPoint:function(e,
g){if(e.length==0||e[0].length<4)return false;for(var j=0,h=e[0][0],n=1,t;n<e[0].length;n++){t=e[0][n];if(h[1]<=g[1]&&g[1]<t[1]||t[1]<=g[1]&&g[1]<h[1]&&(g[0]<h[0]||g[0]<t[0])){h=h[0]+(t[0]-h[0])*(g[1]-h[1])/(t[1]-h[1]);h>g[0]&&j++}h=t}return j%2==1},_containsPolygonLineString:function(e,g){for(var j=0;j<g.length;j++)if(!this._containsPolygonPoint(e,g[j]))return false;return true},_distance:function(e,g){var j=c.isArray(e)?e:e.coordinates,h=c.isArray(g)?g:g.coordinates;j=c.geo.proj?c.geo.proj.fromGeodetic(j):
j;h=c.geo.proj?c.geo.proj.fromGeodetic(h):h;switch(e.type){case "Point":switch(g.type){case "Point":return this._distancePointPoint(h,j);case "LineString":return this._distanceLineStringPoint(h,j);case "Polygon":return this._containsPolygonPoint(h,j)?0:this._distanceLineStringPoint(h[0],j);default:throw Error("not implemented");}case "LineString":switch(g.type){case "Point":return this._distanceLineStringPoint(j,h);case "LineString":return this._distanceLineStringLineString(j,h);case "Polygon":return this._containsPolygonLineString(h,
j)?0:this._distanceLineStringLineString(h[0],j);default:throw Error("not implemented");}case "Polygon":switch(g.type){case "Point":return this._containsPolygonPoint(j,h)?0:this._distanceLineStringPoint(j[0],h);case "LineString":return this._containsPolygonLineString(j,h)?0:this._distanceLineStringLineString(j[0],h);case "Polygon":return this._containsPolygonLineString(j,h[0])?0:this._distanceLineStringLineString(j[0],h[0]);default:throw Error("not implemented");}}},_distancePointPoint:function(e,
g){var j=g[0]-e[0],h=g[1]-e[1];return Math.sqrt(j*j+h*h)},_distanceLineStringPoint:function(e,g){var j=C;if(e.length>0){var h=e[0],n=g[0]-h[0],t=g[1]-h[1];if(e.length==1)return Math.sqrt(n*n+t*t);else for(var l=1;l<e.length;l++){var m=e[l],s=g[0]-m[0],A=g[1]-m[1];h=this._distanceSegmentPoint(m[0]-h[0],m[1]-h[1],n,t,s,A);if(h==0)return 0;if(h<j)j=h;h=m;n=s;t=A}}return Math.sqrt(j)},_distanceSegmentPoint:function(e,g,j,h,n,t){var l=e*j+g*h;if(l<=0)return j*j+h*h;e=e*e+g*g;if(l>=e)return n*n+t*t;return j*
j+h*h-l*l/e},_distanceLineStringLineString:function(e,g){for(var j=C,h=0;h<g;h++)j=Math.min(j,this._distanceLineStringPoint(e,g[h]));return j},_flatten:function(e){var g=[];switch(e.type){case "Feature":c.merge(g,this._flatten(e.geometry));break;case "GeometryCollection":for(curGeom=0;curGeom<e.geometries.length;curGeom++)c.merge(g,this._flatten(e.geometries[curGeom]));break;default:g[0]=e}return g},proj:function(){return{fromGeodeticPos:function(e){return[6378137*e[0]*0.017453292519943295,6378137*
Math.log(Math.tan(0.7853981633974483+e[1]*0.017453292519943295/2))]},fromGeodetic:function(e){var g=c.isArray(e[0]),j=g&&c.isArray(e[0][0]),h=j&&c.isArray(e[0][0][0]),n=[[[]]],t=this.fromGeodeticPos;if(!h){if(!j){g||(e=[e]);e=[e]}e=[e]}c.each(e,function(l){c.each(this,function(m){c.each(this,function(s){n[l][m][s]=t(this)})})});return h?n:j?n[0]:g?n[0][0]:n[0][0][0]},toGeodeticPos:function(e){return[e[0]/6378137*57.29577951308232,(1.5707963267948966-2*Math.atan(1/Math.exp(e[1]/6378137)))*57.29577951308232]},
toGeodetic:function(e){var g=c.isArray(e[0]),j=g&&c.isArray(e[0][0]),h=j&&c.isArray(e[0][0][0]),n=[[[]]],t=this.toGeodeticPos;if(!h){if(!j){g||(e=[e]);e=[e]}e=[e]}c.each(e,function(l){c.each(this,function(m){c.each(this,function(s){n[l][m][s]=t(this)})})});return h?n:j?n[0]:g?n[0][0]:n[0][0][0]}}}()}})(jQuery,this);
