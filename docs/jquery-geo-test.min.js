...
(function(d,n){var y,g={},e,f,h,j,q=[],t,o,B,K,J=null,F=0,v,C,N,ba,Y,H,ha,ia,fa,ca,ga,O,da,la=function(){for(var a=5,b=document.createElement("div"),c=b.all||[];b.innerHTML="<!--[if gt IE "+ ++a+"]><br><![endif]--\>",c[0];);return a>6?a:!a}(),Z,ja=this._supportTouch||this._ieVersion==7,ea,aa,V={},s={},ma={bbox:[-180,-85,180,85],bboxMax:[-180,-85,180,85],center:[0,0],cursors:{pan:"move"},mode:"pan",services:[{id:"OSM",type:"tiled",getUrl:function(a){return"http://tile.openstreetmap.org/"+a.zoom+"/"+
...
(c.visible===n||c.visible?"block":"none")+";'></div>");a[c.id].serviceContainer=f.children("[data-service='"+c.id+"']")}},destroy:function(b,c){a[c.id].serviceContainer.remove();delete a[c.id]},interactivePan:function(b,c,k,m){if(a[c.id]){this._cancelUnloaded(b,c);a[c.id].serviceContainer.children().css({left:function(U,ka){return parseInt(ka)+k},top:function(U,ka){return parseInt(ka)+m}});if(c&&a[c.id]!=null&&(c.visible===n||c.visible)){var l=o,u=a[c.id],x=u.serviceContainer,w=x.children("[data-pixelSize='"+
...
0}}).attr("src",k)}},_cancelUnloaded:function(b,c){var k=a[c.id];if(k&&k.loadCount>0)for(k.serviceContainer.find("img:hidden").remove();k.loadCount>0;)k.loadCount--}}}()}};d.widget("geo.geomap",function(){return{options:d.extend({},ma),_createWidget:function(a,b){V=a;y=d(b);var c=y.css("position");c!="relative"&&c!="absolute"&&c!="fixed"&&y.css("position","relative");y.css("text-align","left");c=this._findMapSize();g={x:parseInt(y.css("padding-left")),y:parseInt(y.css("padding-top")),width:c.width,
height:c.height};this._createChildren();t=B=[0,0];o=K=156543.03392799935;v=C=N=ba=ga=ea=aa=false;Y=H=fa=ca=O=[0,0];da=[0.8,0.8];ha=ia=0;d.Widget.prototype._createWidget.apply(this,arguments)},_create:function(){s=this.options;var a=(Z="ontouchend"in document)?"touchstart":"mousedown",b=Z?"touchend touchcancel":"mouseup",c=Z?"touchmove":"mousemove";j.dblclick(d.proxy(this._eventTarget_dblclick,this));j.bind(a,d.proxy(this._eventTarget_touchstart,this));a=j[0].setCapture?j:d(document);a.bind(c,d.proxy(this._dragTarget_touchmove,
...
a)/Math.log(2)),0)},_getWheelCenterAndSize:function(){var a;if(s.tilingScheme){a=this._getTiledZoom(o)+F;a=this._getTiledPixelSize(a)}else{a=Math.pow(1.18920711500273,-F);a=o*a}var b=a/o,c=this._toMap(Y);b=[(t[0]-c[0])*b,(t[1]-c[1])*b];return{pixelSize:a,center:[c[0]+b[0],c[1]+b[1]]}},_mouseWheelFinish:function(){J=null;if(F!=0){var a=this._getWheelCenterAndSize();F=0;this._setCenterAndSize(a.center,a.pixelSize,true,true)}else this._refresh()},_panEnd:function(){O=[O[0]>0?Math.floor(O[0]*da[0]):Math.ceil(O[0]*
da[0]),O[1]>0?Math.floor(O[1]*da[1]):Math.ceil(O[1]*da[1])];if(Math.abs(O[0])<4&&Math.abs(O[1])<4)this._panFinalize();else{H=[H[0]+O[0],H[1]+O[1]];this._panMove();setTimeout(d.proxy(this._panEnd,this),30)}},_panFinalize:function(){if(ga){O=[0,0];this._setCenterAndSize([t[0]+-(H[0]-Y[0])*o,t[1]+(H[1]-Y[1])*o],o,true,true);C=false;Y=H;N=ga=false;j.css("cursor",s.cursors[s.mode])}},_panMove:function(){var a=H[0]-ca[0],b=H[1]-ca[1];if(N||a>3||a<-3||b>3||b<-3){if(!N){N=true;j.css("cursor",s.cursors.pan)}if(v)O=
[a,b];if(a!=0||b!=0){ga=true;ca=H;for(i=0;i<s.services.length;i++){var c=s.services[i];s._serviceTypes[c.type].interactivePan(this,c,a,b)}}}},_refresh:function(){for(var a=0;a<s.services.length;a++){var b=s.services[a];!v&&s._serviceTypes[b.type]!=null&&s._serviceTypes[b.type].refresh(this,b)}},_setCenterAndSize:function(a,b,c,k){if(o!=b)for(var m=0;m<s.services.length;m++){var l=s.services[m];s._serviceTypes[l.type].interactiveScale(this,l,a,b)}t=a;o=b;if(d.geo.proj){a=this._getBbox();a=d.geo.proj.toGeodetic([[a[0],
...
b,c,k)}else this._setCenterAndSize(a,m,c,k)},_eventTarget_dblclick:function(a){this._panFinalize();d(a.currentTarget).offset();switch(s.mode){case "pan":this._trigger("dblclick",a,{pixels:H,coordinates:this.toMap(H)});a.isDefaultPrevented()||this._zoomTo(this._toMap(H),this._getZoom()+1,true,true)}C=false},_eventTarget_touchstart:function(a){if(!(!Z&&a.which!=1)){if(ja){var b=d.now();aa=b-ha<750?aa?false:ea:false;ea=true;ha=b}a.preventDefault();this._panFinalize();this._mouseWheelFinish();b=d(a.currentTarget).offset();
H=Z?[a.originalEvent.changedTouches[0].pageX-b.left,a.originalEvent.changedTouches[0].pageY-b.top]:[a.pageX-b.left,a.pageY-b.top];v=true;Y=H;if(!C&&a.shiftKey){ba=true;j.css("cursor",s.cursors.zoom)}else{C=true;switch(s.mode){case "pan":ca=H;a.currentTarget.setCapture&&a.currentTarget.setCapture()}}return false}},_dragTarget_touchmove:function(a){var b=j.offset();b=Z?[a.originalEvent.changedTouches[0].pageX-b.left,a.originalEvent.changedTouches[0].pageY-b.top]:[a.pageX-b.left,a.pageY-b.top];if(!(b[0]==
fa[0]&&b[1]==fa[1])){if(ja)aa=ea=false;if(v){H=b;d.now()}switch(ba?"zoom":s.mode){case "pan":v||N?this._panMove():this._trigger("move",a,{pixels:b,coordinates:this.toMap(b)})}fa=b}},_dragTarget_touchstop:function(a){!v&&la==7&&this._eventTarget_touchstart(a);var b=v,c=N,k=j.offset();k=Z?[a.originalEvent.changedTouches[0].pageX-k.left,a.originalEvent.changedTouches[0].pageY-k.top]:[a.pageX-k.left,a.pageY-k.top];var m=ba?"zoom":s.mode;j.css("cursor",s.cursors[m]);ba=v=N=false;document.releaseCapture&&
document.releaseCapture();if(b){b=d.now();H=k;switch(m){case "pan":if(c)this._panEnd();else if(b-ia>100){this._trigger("click",a,{pixels:k,coordinates:this.toMap(k)});C=false}}ia=b;if(ja&&aa){aa=ea=false;j.trigger("dblclick",a)}}},_eventTarget_mousewheel:function(a,b){a.preventDefault();this._panFinalize();if(!v){if(b!=0){if(J){window.clearTimeout(J);J=null}else{var c=d(a.currentTarget).offset();Y=[a.pageX-c.left,a.pageY-c.top]}F+=b;c=this._getWheelCenterAndSize();for(i=0;i<s.services.length;i++){var k=
