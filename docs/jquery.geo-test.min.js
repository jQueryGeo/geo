...
"mousemove";this._$eventTarget.dblclick(e.proxy(this._eventTarget_dblclick,this));this._$eventTarget.keydown(e.proxy(this._eventTarget_keydown,this));this._$eventTarget.bind(a,e.proxy(this._eventTarget_touchstart,this));a=this._$eventTarget[0].setCapture?this._$eventTarget:e(document);a.bind(c,e.proxy(this._dragTarget_touchmove,this));a.bind(b,e.proxy(this._dragTarget_touchstop,this));this._$eventTarget.mousewheel(e.proxy(this._eventTarget_mousewheel,this));var d=this;this._windowHandler=function(){d._resizeTimeout&&
...
0)this._options.mode=="drawLineString"?this._$drawContainer.geographics("drawLineString",this._drawPixels):this._$drawContainer.geographics("drawPolygon",this._drawPixels)},_resetDrawing:function(){this._drawPixels=[];this._drawCoords=[];this._$drawContainer.geographics("clear")},_refreshShapes:function(a,b,c,d,f){var h,l,k,n,p,x=this._getBbox(d,f),u=this;for(h=0;h<b.length;h++){k=b[h].shape||b[h];k=k.geometry||k;l=e.data(k,"geoBbox");if(!(l&&!e.geo._in(x,l))){n=e.isArray(c)?c[h].style:c;switch(k.type){case "Point":this._$shapesContainer.geographics("drawPoint",
this.toPixel(k.coordinates,d,f),n);break;case "LineString":this._$shapesContainer.geographics("drawLineString",this.toPixel(k.coordinates,d,f),n);break;case "Polygon":p=[];e.each(k.coordinates,function(y){p[y]=u.toPixel(this,d,f)});this._$shapesContainer.geographics("drawPolygon",p,n);break;case "MultiPoint":for(l=0;l<k.coordinates;l++)this._$shapesContainer.geographics("drawPoint",this.toPixel(k.coordinates[l],d,f),n);break;case "MultiLineString":for(l=0;l<k.coordinates;l++)this._$shapesContainer.geographics("drawLineString",
...
this._options.tilingScheme;if(b.pixelSizes!=null){a=Math.floor(a*1E3);for(var c=(b.pixelSizes!=null?b.pixelSizes.length:b.levels)-1;c>=0;c--)if(Math.floor(b.pixelSizes[c]*1E3)>=a)return c;return 0}else return Math.max(Math.round(Math.log(b.basePixelSize/a)/Math.log(2)),0)},_getZoomCenterAndSize:function(a,b,c){if(this._options.tilingScheme){b=this._getTiledZoom(this._pixelSize)+b;b=this._getTiledPixelSize(b)}else{b=Math.pow(c,-b);b=this._pixelSize*b}c=b/this._pixelSize;a=this._toMap(a);c=[(this._center[0]-
a[0])*c,(this._center[1]-a[1])*c];return{pixelSize:b,center:[a[0]+c[0],a[1]+c[1]]}},_mouseWheelFinish:function(){this._wheelTimeout=null;if(this._wheelLevel!=0){var a=this._getZoomCenterAndSize(this._anchor,this._wheelLevel,this._wheelZoomFactor);this._setCenterAndSize(a.center,a.pixelSize,true,true);this._wheelLevel=0}else this._refresh()},_panEnd:function(){this._velocity=[this._velocity[0]>0?Math.floor(this._velocity[0]*this._friction[0]):Math.ceil(this._velocity[0]*this._friction[0]),this._velocity[1]>
0?Math.floor(this._velocity[1]*this._friction[1]):Math.ceil(this._velocity[1]*this._friction[1])];if(Math.abs(this._velocity[0])<4&&Math.abs(this._velocity[1])<4)this._panFinalize();else{this._current=[this._current[0]+this._velocity[0],this._current[1]+this._velocity[1]];this._panMove();setTimeout(e.proxy(this._panEnd,this),30)}},_panFinalize:function(){if(this._panning){this._velocity=[0,0];var a=-(this._current[0]-this._anchor[0])*this._pixelSize,b=(this._current[1]-this._anchor[1])*this._pixelSize;
this._$shapesContainer.css({left:0,top:0});this._setCenterAndSize([this._center[0]+a,this._center[1]+b],this._pixelSize,true,true);this._inOp=false;this._anchor=this._current;this._toolPan=this._panning=false;this._$eventTarget.css("cursor",this._options.cursors[this._options.mode])}},_panMove:function(){var a=this._current[0]-this._lastDrag[0],b=this._current[1]-this._lastDrag[1],c=0,d;if(this._toolPan||a>3||a<-3||b>3||b<-3){if(!this._toolPan){this._toolPan=true;this._$eventTarget.css("cursor",this._options.cursors.pan)}if(this._mouseDown)this._velocity=
[a,b];if(a!=0||b!=0){this._panning=true;this._lastDrag=this._current;for(c=0;c<this._options.services.length;c++){d=this._options.services[c];e.geo._serviceTypes[d.type].interactivePan(this,d,a,b)}this._$shapesContainer.css({left:function(f,h){return parseInt(h)+a},top:function(f,h){return parseInt(h)+b}});for(c=0;c<this._drawPixels.length;c++){this._drawPixels[c][0]+=a;this._drawPixels[c][1]+=b}this._refreshDrawing()}}},_refresh:function(){for(var a=0;a<this._options.services.length;a++){var b=this._options.services[a];
!this._mouseDown&&e.geo._serviceTypes[b.type]!=null&&e.geo._serviceTypes[b.type].refresh(this,b)}if(this._$shapesContainer){this._$shapesContainer.geographics("clear");this._graphicShapes.length>0&&this._refreshShapes(this._$shapesContainer,this._graphicShapes,this._graphicShapes)}},_setCenterAndSize:function(a,b,c,d){if(this._pixelSize!=b){this._$shapesContainer.geographics("clear");for(var f=0;f<this._options.services.length;f++){var h=this._options.services[f];e.geo._serviceTypes[h.type].interactiveScale(this,
...
if(d){this._refresh();this._refreshDrawing()}},_toMap:function(a,b,c){var d=e.isArray(a[0]);d||(a=[a]);b=b||this._center;c=c||this._pixelSize;var f=this._contentBounds.width,h=this._contentBounds.height,l=f/2*c;c=h/2*c;var k=[b[0]-l,b[1]-c,b[0]+l,b[1]+c],n=e.geo.width(k,true)/f,p=e.geo.height(k,true)/h,x=[];e.each(a,function(u){x[u]=[k[0]+this[0]*n,k[3]-this[1]*p]});return d?x:x[0]},_toPixel:function(a,b,c){var d=e.isArray(a[0]);d||(a=[a]);b=b||this._center;c=c||this._pixelSize;var f=this._contentBounds.width,
h=this._contentBounds.height,l=f/2*c;c=h/2*c;var k=[b[0]-l,b[1]-c,b[0]+l,b[1]+c],n=e.geo.width(k,true),p=e.geo.height(k,true),x=[];e.each(a,function(u){x[u]=[Math.round((this[0]-k[0])*f/n),Math.round((k[3]-this[1])*h/p)]});return d?x:x[0]},_zoomTo:function(a,b,c,d){b=b<0?0:b;var f=this._getTiledPixelSize(b);if(isNaN(f)){b=e.geo._scaleBy(this._getBboxMax(),1/Math.pow(this._zoomFactor,b),true);b=Math.max(e.geo.width(b,true)/this._contentBounds.width,e.geo.height(b,true)/this._contentBounds.height);
this._setCenterAndSize(a,b,c,d)}else this._setCenterAndSize(a,f,c,d)},_eventTarget_dblclick_zoom:function(a){this._trigger("dblclick",a,{type:"Point",coordinates:this.toMap(this._current)});if(!a.isDefaultPrevented()){a=this._getZoomCenterAndSize(this._current,1,this._zoomFactor);this._setCenterAndSize(a.center,a.pixelSize,true,true)}},_eventTarget_dblclick:function(a){this._panFinalize();if(this._drawTimeout){window.clearTimeout(this._drawTimeout);this._drawTimeout=null}e(a.currentTarget).offset();
switch(this._options.mode){case "pan":case "drawPoint":this._eventTarget_dblclick_zoom(a);break;case "drawLineString":if(this._drawCoords.length>1&&!(this._drawCoords[0][0]==this._drawCoords[1][0]&&this._drawCoords[0][1]==this._drawCoords[1][1])){this._drawCoords.length--;this._trigger("shape",a,{type:"LineString",coordinates:e.geo.proj?e.geo.proj.toGeodetic(this._drawCoords):this._drawCoords})}else this._eventTarget_dblclick_zoom(a);this._resetDrawing()}this._inOp=false},_eventTarget_keydown:function(a){if(this._drawCoords.length>
0&&a.which==27)if(this._drawCoords.length<=2){this._resetDrawing();this._inOp=false}else{this._drawCoords[this._drawCoords.length-2]=this._drawCoords[this._drawCoords.length-1];this._drawCoords.length-=1;this._drawPixels[this._drawPixels.length-2]=this._drawPixels[this._drawPixels.length-1];this._drawPixels.length-=1;this._refreshDrawing()}},_eventTarget_touchstart:function(a){if(!(!this._supportTouch&&a.which!=1)){this._panFinalize();this._mouseWheelFinish();var b=e(a.currentTarget).offset();this._current=
this._supportTouch?[a.originalEvent.changedTouches[0].pageX-b.left,a.originalEvent.changedTouches[0].pageY-b.top]:[a.pageX-b.left,a.pageY-b.top];if(this._softDblClick){b=e.now();if(b-this._downDate<750){if(this._isTap){var c=this._current[0]-this._anchor[0],d=this._current[1]-this._anchor[1];if(Math.sqrt(c*c+d*d)>10)this._isTap=false;else this._current=this._anchor}this._isDbltap=this._isDbltap?false:this._isTap}else this._isDbltap=false;this._isTap=true;this._downDate=b}a.preventDefault();this._mouseDown=
true;this._anchor=this._current;if(!this._inOp&&a.shiftKey){this._shiftZoom=true;this._$eventTarget.css("cursor",this._options.cursors.zoom)}else{this._inOp=true;switch(this._options.mode){case "pan":case "drawPoint":case "drawLineString":this._lastDrag=this._current;a.currentTarget.setCapture&&a.currentTarget.setCapture()}}return false}},_dragTarget_touchmove:function(a){var b=this._$eventTarget.offset(),c=this._drawCoords.length;b=this._supportTouch?[a.originalEvent.changedTouches[0].pageX-b.left,
a.originalEvent.changedTouches[0].pageY-b.top]:[a.pageX-b.left,a.pageY-b.top];if(!(b[0]==this._lastMove[0]&&b[1]==this._lastMove[1])){if(this._softDblClick)this._isDbltap=this._isTap=false;if(this._mouseDown){this._current=b;this._moveDate=e.now()}switch(this._shiftZoom?"zoom":this._options.mode){case "pan":case "drawPoint":this._mouseDown||this._toolPan?this._panMove():this._trigger("move",a,{type:"Point",coordinates:this.toMap(b)});break;case "drawLineString":if(this._mouseDown||this._toolPan)this._panMove();
else{if(c>0){this._drawCoords[c-1]=this._toMap(b);this._drawPixels[c-1]=b;this._refreshDrawing()}this._trigger("move",a,{type:"Point",coordinates:this.toMap(b)})}}this._lastMove=b}},_dragTarget_touchstop:function(a){!this._mouseDown&&q==7&&this._eventTarget_touchstart(a);var b=this._mouseDown,c=this._toolPan,d=this._$eventTarget.offset(),f=this._shiftZoom?"zoom":this._options.mode,h;h=this._supportTouch?[a.originalEvent.changedTouches[0].pageX-d.left,a.originalEvent.changedTouches[0].pageY-d.top]:
[a.pageX-d.left,a.pageY-d.top];this._$eventTarget.css("cursor",this._options.cursors[f]);this._shiftZoom=this._mouseDown=this._toolPan=false;document.releaseCapture&&document.releaseCapture();if(b){b=e.now();this._current=h;switch(f){case "pan":if(c)this._panEnd();else if(b-this._clickDate>100){this._trigger("click",a,{type:"Point",coordinates:this.toMap(h)});this._inOp=false}break;case "drawPoint":if(this._drawTimeout){window.clearTimeout(this._drawTimeout);this._drawTimeout=null}if(c)this._panEnd();
else if(b-this._clickDate>100){var l=this;this._drawTimeout=setTimeout(function(){if(l._drawTimeout){l._trigger("shape",a,{type:"Point",coordinates:l.toMap(h)});l._inOp=false;l._drawTimeout=false}},250)}break;case "drawLineString":if(c)this._panEnd();else{c=this._drawCoords.length==0?0:this._drawCoords.length-1;this._drawCoords[c]=this._toMap(h);this._drawPixels[c]=h;if(c<2||!(this._drawCoords[c][0]==this._drawCoords[c-1][0]&&this._drawCoords[c][1]==this._drawCoords[c-1][1])){this._drawCoords[c+1]=
