...
true;this._multiTouchAnchor.push(d[0]);this._multiTouchCurrentBbox=[this._multiTouchCurrentBbox[0],this._multiTouchCurrentBbox[1],this._multiTouchAnchor[1].pageX-c.left,this._multiTouchAnchor[1].pageY-c.top];this._multiTouchAnchorBbox=g.merge([],this._multiTouchCurrentBbox);this._mouseDown=true;this._anchor=this._current=g.geo.center(this._multiTouchCurrentBbox,true);return false}if(this._isMultiTouch){for(;e<d.length;e++)if(d[e].identifier===this._multiTouchAnchor[0].identifier){this._multiTouchCurrentBbox[0]=
d[e].pageX-c.left;this._multiTouchCurrentBbox[1]=d[e].pageY-c.top}else if(d[e].identifier===this._multiTouchAnchor[1].identifier){this._multiTouchCurrentBbox[2]=d[e].pageX-c.left;this._multiTouchCurrentBbox[3]=d[e].pageY-c.top}g.geo.center(this._multiTouchCurrentBbox,true);e=this._multiTouchCurrentBbox[2]-this._multiTouchCurrentBbox[0];c=this._multiTouchAnchorBbox[2]-this._multiTouchAnchorBbox[0];this._wheelLevel=Math.abs(Math.floor((1-e/c)*10));if(Math.abs(e)<Math.abs(c))this._wheelLevel=-this._wheelLevel;
d=this._getZoomCenterAndSize(this._anchor,this._wheelLevel,false);this._$elem.find(".geo-shapes-container").geographics("clear");for(e=0;e<this._currentServices.length;e++){c=this._currentServices[e];g.geo._serviceTypes[c.type].interactiveScale(this,c,d.center,d.pixelSize)}this._graphicShapes.length>0&&this._graphicShapes.length<256&&this._refreshShapes(this._$shapesContainer,this._graphicShapes,this._graphicShapes,this._graphicShapes,d.center,d.pixelSize);if(this._drawCoords.length>0){this._drawPixels=
this._toPixel(this._drawCoords,d.center,d.pixelSize);this._refreshDrawing()}e=g.geo.center(this._multiTouchCurrentBbox,true)}else e=[b.originalEvent.changedTouches[0].pageX-c.left,b.originalEvent.changedTouches[0].pageY-c.top]}else e=[b.pageX-c.left,b.pageY-c.top];if(e[0]===this._lastMove[0]&&e[1]===this._lastMove[1])if(this._inOp){b.preventDefault();return false}if(r==7)this._isDbltap=this._isTap=false;if(this._mouseDown){this._current=e;this._moveDate=g.now()}if(this._isMultiTouch){b.preventDefault();
return this._isDbltap=this._isTap=false}switch(this._shiftZoom?"zoom":this._options.mode){case "zoom":if(this._mouseDown){this._$drawContainer.geographics("clear");this._$drawContainer.geographics("drawBbox",[this._anchor[0],this._anchor[1],e[0],e[1]])}else this._trigger("move",b,{type:"Point",coordinates:this.toMap(e)});break;case "drawLineString":case "drawPolygon":case "measureLength":case "measureArea":if(this._mouseDown||this._toolPan)this._panMove();else{if(a>0){this._drawCoords[a-1]=this._toMap(e);
this._drawPixels[a-1]=e;this._refreshDrawing()}this._trigger("move",b,{type:"Point",coordinates:this.toMap(e)})}break;default:this._mouseDown||this._toolPan?this._panMove():this._trigger("move",b,{type:"Point",coordinates:this.toMap(e)})}this._lastMove=e;if(this._inOp){b.preventDefault();return false}}},_dragTarget_touchstop:function(b){if(this._options.mode!=="static"){!this._mouseDown&&r==7&&this._eventTarget_touchstart(b);var c=this._mouseDown,a=this._toolPan,d=this._$eventTarget.offset(),e=this._shiftZoom?
"zoom":this._options.mode,f,l;f=this._supportTouch?[b.originalEvent.changedTouches[0].pageX-d.left,b.originalEvent.changedTouches[0].pageY-d.top]:[b.pageX-d.left,b.pageY-d.top];if(this._softDblClick)if(this._isTap){d=f[0]-this._anchor[0];l=f[1]-this._anchor[1];if(Math.sqrt(d*d+l*l)<=8)f=g.merge([],this._anchor)}d=f[0]-this._anchor[0];l=f[1]-this._anchor[1];this._$eventTarget.css("cursor",this._options.cursors[this._options.mode]);this._shiftZoom=this._mouseDown=this._toolPan=false;if(this._isMultiTouch){b.preventDefault();
this._isMultiTouch=false;c=this._getZoomCenterAndSize(this._anchor,this._wheelLevel,false);this._setCenterAndSize(c.center,c.pixelSize,true,true);this._wheelLevel=0;return false}document.releaseCapture&&document.releaseCapture();if(c){c=g.now();this._current=f;switch(e){case "zoom":if(d>0||l>0){a=this._pixelSize*6;e=this._toMap([[Math.min(this._anchor[0],f[0]),Math.max(this._anchor[1],f[1])],[Math.max(this._anchor[0],f[0]),Math.min(this._anchor[1],f[1])]]);e=[e[0][0],e[0][1],e[1][0],e[1][1]];if(e[2]-
e[0]<a&&e[3]-e[1]<a)e=g.geo.scaleBy(this._getBbox(g.geo.center(e,true)),0.5,true);this._setBbox(e,true,true)}this._resetDrawing();break;case "drawPoint":if(this._drawTimeout){window.clearTimeout(this._drawTimeout);this._drawTimeout=null}if(a)this._panFinalize();else if(c-this._clickDate>100){var m=this;this._drawTimeout=setTimeout(function(){if(m._drawTimeout){m._trigger("shape",b,{type:"Point",coordinates:m.toMap(f)});m._inOp=false;m._drawTimeout=null}},250)}break;case "drawLineString":case "drawPolygon":case "measureLength":case "measureArea":if(a)this._panFinalize();
