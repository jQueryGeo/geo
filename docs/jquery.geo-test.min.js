...
width:a.width,height:a.height};this._$contentFrame.css({width:a.width,height:a.height});this._$servicesContainer.css({width:a.width,height:a.height});this._$eventTarget.css({width:a.width,height:a.height});this._$drawContainer.geographics("destroy");this._$shapesContainer.geographics("destroy");for(e=0;e<this._currentServices.length;e++)f.geo._serviceTypes[this._currentServices[e].type].resize(this,this._currentServices[e]);this._$panContainer.css({width:a.width,height:a.height});this._$shapesContainer.geographics({style:d});
...
this._toolPan=false;document.releaseCapture&&document.releaseCapture();if(c){c=f.now();this._current=e;switch(g){case "zoom":if(d>0||p>0){b=this._pixelSize*6;g=this._toMap([[Math.min(this._anchor[0],e[0]),Math.max(this._anchor[1],e[1])],[Math.max(this._anchor[0],e[0]),Math.min(this._anchor[1],e[1])]]);g=[g[0][0],g[0][1],g[1][0],g[1][1]];if(g[2]-g[0]<b&&g[3]-g[1]<b)g=f.geo.scaleBy(this._getBbox(f.geo.center(g,true)),0.5,true);this._setBbox(g,true,true)}this._resetDrawing();break;case "pan":if(b)this._panEnd();
else if(c-this._clickDate>100){this._trigger("click",a,{type:"Point",coordinates:this.toMap(e)});this._inOp=false}break;case "drawPoint":if(this._drawTimeout){window.clearTimeout(this._drawTimeout);this._drawTimeout=null}if(b)this._panFinalize();else if(c-this._clickDate>100){var l=this;this._drawTimeout=setTimeout(function(){if(l._drawTimeout){l._trigger("shape",a,{type:"Point",coordinates:l.toMap(e)});l._inOp=false;l._drawTimeout=false}},250)}break;case "drawLineString":case "drawPolygon":case "measureLength":case "measureArea":if(b)this._panFinalize();
else{b=this._drawCoords.length==0?0:this._drawCoords.length-1;this._drawCoords[b]=this._toMap(e);this._drawPixels[b]=e;if(b<2||!(this._drawCoords[b][0]==this._drawCoords[b-1][0]&&this._drawCoords[b][1]==this._drawCoords[b-1][1])){this._drawCoords[b+1]=this._toMap(e);this._drawPixels[b+1]=e}this._refreshDrawing()}}this._clickDate=c;if(this._softDblClick&&this._isDbltap){this._isDbltap=this._isTap=false;this._$eventTarget.trigger("dblclick",a)}}if(this._inOp){a.preventDefault();return false}},_eventTarget_mousewheel:function(a,
...
f.geo._serviceTypes[d.type].interactiveScale(this,d,b.center,b.pixelSize)}this._$shapesContainer.geographics("clear");this._$labelsContainer.html("");this._graphicShapes.length>0&&this._graphicShapes.length<256&&this._refreshShapes(this._$shapesContainer,this._graphicShapes,this._graphicShapes,this._graphicShapes,b.center,b.pixelSize);if(this._drawCoords.length>0){this._drawPixels=this._toPixel(this._drawCoords,b.center,b.pixelSize);this._refreshDrawing()}var g=this;this._wheelTimeout=window.setTimeout(function(){g._mouseWheelFinish()},
