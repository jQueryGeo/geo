...
0],cursors:{pan:"move",zoom:"crosshair",drawPoint:"crosshair",drawLineString:"crosshair",drawPolygon:"crosshair",measureDistance:"crosshair"},measureLabels:{distance:"{{=distance.toFixed( 2 )}} m",area:"{{=area.toFixed( 2 )}} sq m"},drawStyle:{},shapeStyle:{},mode:"pan",services:[{"class":"osm",type:"tiled",getUrl:function(a){return"http://tile.openstreetmap.org/"+a.zoom+"/"+a.tile.column+"/"+a.tile.row+".png"},attr:"&copy; OpenStreetMap &amp; contributors, CC-BY-SA"}],tilingScheme:{tileWidth:256,
...
b)}this._graphicShapes.push({shape:c[d],style:g,label:e})}if(p===w||p)this._refresh()}},empty:function(a){f.each(this._graphicShapes,function(){f.removeData(this,"geoBbox")});this._graphicShapes=[];if(a===w||a)this._refresh()},find:function(a,c){this.toPixel(a.coordinates);var b=this._pixelSize*c,d=[],g;f.each(this._graphicShapes,function(){if(this.shape.type=="Point")f.geo.distance(this.shape,a)<=b&&d.push(this.shape);else{var e=f.data(this.shape,"geoBbox");e={type:"Polygon",coordinates:[[[e[0],
...
this._options.services[a];this._currentServices[a].serviceContainer=f.geo._serviceTypes[this._currentServices[a].type].create(this,this._$servicesContainer,this._currentServices[a],a).geomap()}},_refreshDrawing:function(){this._$drawContainer.geographics("clear");if(this._drawPixels.length>0){var a=this._options.mode,c=this._drawPixels,b=this._drawCoords,d,g;switch(a){case "measureDistance":a="drawLineString";g={type:"LineString",coordinates:b};d=f.render({distance:f.geo.length(g,true)},"geoMeasureDistance");
g=f.merge([],c[c.length-1]);break;case "drawPolygon":c=[c]}this._$drawContainer.geographics(a,c);if(d){this._$measureLabel.html(d);a=this._contentBounds.width-(this._$measureLabel.outerWidth(true)+g[0]);c=this._contentBounds.height-(this._$measureLabel.outerHeight(true)+g[1]);if(a<0)g[0]+=a;if(c<0)g[1]+=c;this._$measureLabel.css({left:g[0],top:g[1]}).show()}}},_resetDrawing:function(){this._drawPixels=[];this._drawCoords=[];this._$drawContainer.geographics("clear");this._$measureLabel.hide()},_refreshShapes:function(a,
c,b,d,g,e){var p,l,o,t,s,v,A,H=this._getBbox(g,e);for(p=0;p<c.length;p++){o=c[p].shape||c[p];o=o.geometry||o;l=f.data(o,"geoBbox");if(!(l&&f.geo._bboxDisjoint(H,l))){t=f.isArray(b)?b[p].style:b;s=f.isArray(d)?d[p].label:d;v=s!==w;A=w;switch(o.type){case "Point":A=this.toPixel(o.coordinates,g,e);this._$shapesContainer.geographics("drawPoint",A,t);break;case "LineString":this._$shapesContainer.geographics("drawLineString",this.toPixel(o.coordinates,g,e),t);if(v)A=this.toPixel(f.geo.pointAlong(o,0.5).coordinates,
g,e);break;case "Polygon":this._$shapesContainer.geographics("drawPolygon",this.toPixel(o.coordinates,g,e),t);if(v)A=this.toPixel(f.geo.centroid(o).coordinates,g,e);break;case "MultiPoint":for(l=0;l<o.coordinates.length;l++)this._$shapesContainer.geographics("drawPoint",this.toPixel(o.coordinates[l],g,e),t);if(v)A=this.toPixel(f.geo.centroid(o).coordinates,g,e);break;case "MultiLineString":for(l=0;l<o.coordinates.length;l++)this._$shapesContainer.geographics("drawLineString",this.toPixel(o.coordinates[l],
g,e),t);if(v)A=this.toPixel(f.geo.centroid(o).coordinates,g,e);break;case "MultiPolygon":for(l=0;l<o.coordinates.length;l++)this._$shapesContainer.geographics("drawPolygon",this.toPixel(o.coordinates[l],g,e),t);if(v)A=this.toPixel(f.geo.centroid(o).coordinates,g,e);break;case "GeometryCollection":this._refreshShapes(a,o.geometries,t,s,g,e)}v&&A&&this._$labelsContainer.append('<div class="geo-label" style="position:absolute; left:'+A[0]+"px; top:"+A[1]+'px;">'+s+"</div>")}}},_findMapSize:function(){for(var a=
{width:0,height:0},c=this._$elem;c.size()&&!(a.width>0&&a.height>0);){a={width:c.width(),height:c.height()};if(a.width<=0||a.height<=0)a={width:parseInt(c.css("width")),height:parseInt(c.css("height"))};c=c.parent()}return a},_forcePosition:function(a){var c=a.css("position");c!="relative"&&c!="absolute"&&c!="fixed"&&a.css("position","relative")},_getTiledPixelSize:function(a){var c=this._options.tilingScheme;if(c!=null){if(a===0)return c.pixelSizes!=null?c.pixelSizes[0]:c.basePixelSize;a=Math.round(a);
a=Math.max(a,0);a=Math.min(a,(c.pixelSizes!=null?c.pixelSizes.length:c.levels)-1);return c.pixelSizes!=null?c.pixelSizes[a]:c.basePixelSize/Math.pow(2,a)}else return NaN},_getTiledZoom:function(a){var c=this._options.tilingScheme;if(c.pixelSizes!=null){a=Math.floor(a*1E3);for(var b=c.pixelSizes.length-1;b>=0;b--)if(Math.floor(c.pixelSizes[b]*1E3)>=a)return b;return 0}else return Math.max(Math.round(Math.log(c.basePixelSize/a)/Math.log(2)),0)},_getZoomCenterAndSize:function(a,c,b){if(this._options.tilingScheme){c=
this._getTiledZoom(this._pixelSize)+c;c=this._getTiledPixelSize(c)}else{c=Math.pow(b,-c);c=this._pixelSize*c}b=c/this._pixelSize;a=this._toMap(a);b=[(this._center[0]-a[0])*b,(this._center[1]-a[1])*b];return{pixelSize:c,center:[a[0]+b[0],a[1]+b[1]]}},_mouseWheelFinish:function(){this._wheelTimeout=null;if(this._wheelLevel!=0){var a=this._getZoomCenterAndSize(this._anchor,this._wheelLevel,this._wheelZoomFactor);this._setCenterAndSize(a.center,a.pixelSize,true,true);this._wheelLevel=0}else this._refresh()},
_panEnd:function(){this._velocity=[this._velocity[0]>0?Math.floor(this._velocity[0]*this._friction[0]):Math.ceil(this._velocity[0]*this._friction[0]),this._velocity[1]>0?Math.floor(this._velocity[1]*this._friction[1]):Math.ceil(this._velocity[1]*this._friction[1])];if(Math.abs(this._velocity[0])<4&&Math.abs(this._velocity[1])<4)this._panFinalize();else{this._current=[this._current[0]+this._velocity[0],this._current[1]+this._velocity[1]];this._panMove();setTimeout(f.proxy(this._panEnd,this),30)}},
_panFinalize:function(){if(this._panning){this._velocity=[0,0];var a=-(this._current[0]-this._anchor[0])*this._pixelSize,c=(this._options.axisLayout==="image"?-1:1)*(this._current[1]-this._anchor[1])*this._pixelSize;this._$panContainer.css({left:0,top:0});this._setCenterAndSize([this._center[0]+a,this._center[1]+c],this._pixelSize,true,true);this._inOp=false;this._anchor=this._current;this._toolPan=this._panning=false;this._$eventTarget.css("cursor",this._options.cursors[this._options.mode])}},_panMove:function(){var a=
...
1&&!(this._drawCoords[0][0]==this._drawCoords[1][0]&&this._drawCoords[0][1]==this._drawCoords[1][1])){var c=this._drawCoords.length-1;if(c>2){this._drawCoords[c]=f.merge([],this._drawCoords[0]);this._trigger("shape",a,{type:"Polygon",coordinates:[f.geo.proj?f.geo.proj.toGeodetic(this._drawCoords):this._drawCoords]})}}else this._eventTarget_dblclick_zoom(a);this._resetDrawing();break;case "measureDistance":this._resetDrawing()}this._inOp=false},_eventTarget_touchstart:function(a){if(!(!this._supportTouch&&
...
this._$eventTarget.css("cursor",this._options.cursors[this._options.mode]);this._shiftZoom=this._mouseDown=this._toolPan=false;document.releaseCapture&&document.releaseCapture();if(c){c=f.now();this._current=e;switch(g){case "zoom":if(d>0||p>0){b=this._pixelSize*6;g=this._toMap([[Math.min(this._anchor[0],e[0]),Math.max(this._anchor[1],e[1])],[Math.max(this._anchor[0],e[0]),Math.min(this._anchor[1],e[1])]]);g=[g[0][0],g[0][1],g[1][0],g[1][1]];if(g[2]-g[0]<b&&g[3]-g[1]<b)g=f.geo.scaleBy(this._getBbox(f.geo.center(g,
true)),0.5,true);this._setBbox(g,true,true)}this._resetDrawing();break;case "pan":if(b)this._panEnd();else if(c-this._clickDate>100){this._trigger("click",a,{type:"Point",coordinates:this.toMap(e)});this._inOp=false}break;case "drawPoint":if(this._drawTimeout){window.clearTimeout(this._drawTimeout);this._drawTimeout=null}if(b)this._panFinalize();else if(c-this._clickDate>100){var l=this;this._drawTimeout=setTimeout(function(){if(l._drawTimeout){l._trigger("shape",a,{type:"Point",coordinates:l.toMap(e)});
l._inOp=false;l._drawTimeout=false}},250)}break;case "drawLineString":case "drawPolygon":case "measureDistance":if(b)this._panFinalize();else{b=this._drawCoords.length==0?0:this._drawCoords.length-1;this._drawCoords[b]=this._toMap(e);this._drawPixels[b]=e;if(b<2||!(this._drawCoords[b][0]==this._drawCoords[b-1][0]&&this._drawCoords[b][1]==this._drawCoords[b-1][1])){this._drawCoords[b+1]=this._toMap(e);this._drawPixels[b+1]=e}this._refreshDrawing()}}this._clickDate=c;if(this._softDblClick&&this._isDbltap){this._isDbltap=
...
for(i=0;i<this._options.services.length;i++){var d=this._options.services[i];f.geo._serviceTypes[d.type].interactiveScale(this,d,b.center,b.pixelSize)}this._$shapesContainer.geographics("clear");this._$labelsContainer.html("");this._graphicShapes.length>0&&this._graphicShapes.length<256&&this._refreshShapes(this._$shapesContainer,this._graphicShapes,this._graphicShapes,this._graphicShapes,b.center,b.pixelSize);if(this._drawCoords.length>0){this._drawPixels=this._toPixel(this._drawCoords,b.center,
